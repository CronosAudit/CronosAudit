...
export function HeroWave({
  ...
  useEffect(() => {
    ...
    function createFilmGrainPass(intensity = 0.9, grainScale = 0.3) {
      ...
      (pass.uniforms as Record<string, { value: unknown }>).intensity.value =
        intensity;
      (pass.uniforms as Record<string, { value: unknown }>).grainScale.value =
        grainScale;
      return pass;
    }
    ...
    function updateGlowDistance() {
      ...
      (barMaterial.uniforms as Record<string, { value: unknown }>).uMaxGlowDist.value =
        spanPx;
    }

    function createInstancedMaterial() {
      const baseCol = new THREE.Color("#d4af37");
      const emisCol = new THREE.Color("#f4e7b2");
    ...
    function setupQuickSetters() {
      const u = (instancedBars?.material as THREE.ShaderMaterial).uniforms as Record<
        string,
        { value: unknown }
      >;
    ...
    function updateGainMultiplier() {
      ...
      (barMaterial.uniforms as Record<string, { value: unknown }>).uGainMul.value =
    ...
    function createInstancedBars() {
      ...
      const barCount = Math.min(
        ...
        Math.max(
          1,
          Math.floor((span + FIXED_BAR_GAP) / (FIXED_BAR_WIDTH + FIXED_BAR_GAP))
        )
      );
      const gap =
        barCount > 1
          ? (span - barCount * FIXED_BAR_WIDTH) / (barCount - 1)
          : 0;
    ...
    function buildKeyframeTweens(
      target: { gain: number; frequency: number; waveLength: number },
    ...
    function onResize(newW: number, newH: number) {
      ...
      (barMaterial.uniforms as Record<string, { value: unknown }>).uHalfW.value =
        cameraWidth * 0.5;
    ...
    function applyHeavyResize() {
      ...
      (grainPass.uniforms as Record<string, { value: unknown }>).grainScale.value =
        0.5;
    }
    ...
    const ticker = () => {
      ...
      (grainPass.uniforms as Record<string, { value: unknown }>).time.value =
        ((grainPass.uniforms as Record<string, { value: unknown }>).time.value as number) +
        dt * 0.2;
    ...
    >
      ...
      {showOverlayContent ? (
        ...
        >
          ...
          >
            <h1 className="text-white text-3xl sm:text-5xl font-semibold tracking-tight drop-shadow-[0_1px_8px_rgba(212,175,55,0.25)]">
              {title}
            </h1>
              ...
              <div className="relative w-full sm:w-[720px]">
                <div className="relative rounded-2xl p-[2px] shadow-[0_1px_2px_0_rgba(0,0,0,0.06)] bg-gradient-to-br from-white/10 via-white/5 to-black/20">
                  <textarea
                    ...
                    rows={5}
                    className="w-full h-32 sm:h-36 resize-none rounded-2xl bg-[rgba(15,15,20,0.55)] border border-white/10 text-white placeholder:text-white/40 outline-none focus:ring-2 focus:ring-[#d4af37]/40 focus:border-[#d4af37]/40 backdrop-blur-md px-4 py-4 pr-16"
                  />
                </div>
                ...
                <button
                  ...
                  className="absolute right-3 bottom-3 inline-flex items-center justify-center w-10 h-10 rounded-xl bg-[#f4e7b2] text-black hover:bg-white transition-colors"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
